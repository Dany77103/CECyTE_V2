...
if (isset($_GET['estatus'])) {
    $filtro_estatus = intval($_GET['estatus']);
}
...
try {
    ...
    if (!empty($busqueda)) {
        $sql .= " AND (m.nombre LIKE :busqueda OR 
                      ...
                      m.correo_institucional LIKE :busqueda OR
                      m.rfc LIKE :busqueda OR
                      m.curp LIKE :busqueda)";
    ...
    $estatus_list = $con->query($sql_estatus)->fetchAll(PDO::FETCH_ASSOC);
...
if (isset($_GET['error'])) {
    switch ($_GET['error']) {
        ...
        case 'numero_invalido':
            $mensaje = '<div class="alert alert-danger">N&uacute;mero de empleado inv&aacute;lido.</div>';
            break;
    }
}
?>
...
<head>
    ...
    <div>
        <h2><i class='bx bx-user'></i> Lista de Maestros</h2>
    </div>
    <!--<div>-->
        <!-- Botones de exportación para todos los maestros -->
       <!-- <a href="exportar_excel.php" class="btn btn-success me-2" title="Exportar a Excel">
            <i class='bx bx-file'></i> Exportar Excel
        </a>
        <a href="exportar_pdf.php" class="btn btn-danger me-2" title="Exportar a PDF">
            <i class='bx bxs-file-pdf'></i> Exportar PDF
        </a>
        <a href="agregar_maestro.php" class="btn btn-primary">
        ...
        <div class="filtros-container">
            <form method="GET" action="" class="row g-3">
                <div class="col-md-4">
                    ...
                    <input type="text" class="form-control" id="busqueda" name="busqueda" 
                           ...
                           placeholder="Nombre, apellido, n&uacute;mero de empleado, RFC, CURP o email">
                </div>
                ...
                <div class="col-md-3">
                    <label for="estatus" class="form-label">Estatus Laboral</label>
                    <select class="form-select" id="estatus" name="estatus">
                        <option value="">Todos los estatus</option>
                        <?php foreach ($estatus_list as $estatus): ?>
                            <option value="<?php echo $estatus['id_estatus']; ?>" 
                                <?php echo ($filtro_estatus == $estatus['id_estatus']) ? 'selected' : ''; ?>>
                                <?php echo htmlspecialchars($estatus['tipoEstatus']); ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
                </div>
            ...
            <div class="row mt-3">
                <div class="col-md-12 export-buttons">
                    <div class="btn-group">
                        <a href="exportarM_excel.php?format=excel&<?php echo http_build_query($_GET); ?>" 
                           class="btn btn-outline-success">
                            <i class='bx bx-download'></i> Exportar a Excel
                        </a>
                        <a href="exportarM_pdf.php?format=pdf&<?php echo http_build_query($_GET); ?>" 
        ...
        <div class="table-container">
            <?php if (empty($maestros)): ?>
                <div class="alert alert-info text-center">
                    <i class='bx bx-info-circle'></i> No se encontraron maestros con los criterios de b&uacute;squeda.
                </div>
            <?php else: ?>
                        ...
                        <tbody>
                            ...
                            <?php foreach ($maestros as $maestro): ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php if (!empty($maestro['numCelular'])): ?>
                                            <a href="tel:<?php echo htmlspecialchars($maestro['teleono_celular']); ?>">
                                    ...
                                    <td>
                                        <?php if (!empty($maestro['estatus_laboral'])): ?>
                                            <?php 
                                            $estatus_class = 'badge-active';
                                            if (strpos(strtolower($maestro['estatus_laboral']), 'inactivo') !== false) {
                                                $estatus_class = 'badge-inactive';
                                            } elseif (strpos(strtolower($maestro['estatus_laboral']), 'suspendido') !== false) {
                                                $estatus_class = 'badge-suspended';
                                            }
                                            ?>
                                            <span class="badge badge-status <?php echo $estatus_class; ?>">
                                                <?php echo htmlspecialchars($maestro['estatus_laboral']); ?>
                                            </span>
                                        <?php else: ?>
                                    ...
                                    <td>
                                        <div class="action-buttons">
                                            ...
                                            <a href="editar_datos_academicos.php?numEmpleado=<?php echo urlencode($maestro['numEmpleado']); ?>" 
                                               class="btn btn-primary btn-sm" title="Datos Acad&eacute;micos">
                                                <i class='bx bx-book'></i>
                                            </a>
                                            <a href="editar_datos_laborales.php?numEmpleado=<?php echo urlencode($maestro['numEmpleado']); ?>" 
                ...
                <?php if ($total_paginas > 1): ?>
                    <nav aria-label="Paginaci&oacute;n de maestros">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>¿Est&aacute; seguro que desea eliminar al maestro <strong id="nombreMaestro"></strong>?</p>
                    <p class="text-danger"><strong>Esta acci&oacute;n no se puede deshacer.</strong></p>
                    <p class="text-warning"><small>Nota: Tambi&eacute;n se eliminar&aacute;n sus datos acad&eacute;micos y laborales asociados.</small></p>
                </div>
